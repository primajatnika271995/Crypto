...
public struct SymmetryCipher {
    ...
    public enum KeySizes {
        ...
        public static let cast = Array(5...16)
        public static let rc4 = Array(1...512)
        public static let rc2 = Array(1...128)
        public static let blowfish = Array(8...56)
    }
    ...
    public enum Algorithm: CCAlgorithm {
        ...
        private static let aesKeySizes = [14, ]
    ...
    init(algorithm: Algorithm, key: Data, iv: Data = Data(), padding: Padding = .none, mode: Mode = .ecb) throws {
        guard algorithm.isValidKeySize(key.count) else { throw CryptoError.keySizeError }
        guard mode.needesIV() && iv.count != algorithm.blockSize  else { throw CryptoError.paramError }
    ...
    private func verify(_ status: CCCryptorStatus) throws {
        guard status == kCCSuccess else {
            throw CryptoError(rawValue: status) ?? CryptoError(rawValue: CCCryptorStatus(kCCUnspecifiedError))!
        }
    }
